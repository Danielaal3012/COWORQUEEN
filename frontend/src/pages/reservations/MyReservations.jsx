    ...
    return (
        <div className='p-4'>
            ...
            <ul className='flex flex-col justify-start gap-2 md:flex-row md:flex-wrap'>
                ...
                <figure
                key={reservation.id}
                className="relative px-1 overflow-hidden border rounded-lg md:w-[30%] md:py-1 h-1/2 border-ring bg-vanilla/35 hover:bg-opacity-5"
                >
                <Link to={`/reservation/${reservation.id}`} className='flex flex-row items-center h-full gap-x-2'>
                  <img
                    src={host + "/uploads/rooms/" + reservation?.roomId + "/" + reservation?.roomCover}
                  ...
                  <div className='h-full'>
                    ...
                    <p>{formatDate(reservation?.reservationDateBeg)}</p>
                    <p className='text-sm'>{formatTime(reservation?.reservationDateBeg)} - {formatTime(reservation?.reservationDateEnd)}</p>
                  </div>
</Link>
</figure>      
                ))}
            ...
            <ul className='flex flex-col justify-start gap-2 md:flex-row md:flex-wrap'>
                {pastReservations.map((reservation) => (
                                    ...
                                    className="relative px-1 overflow-hidden border rounded-lg md:w-[30%] md:py-1 h-1/2 border-ring bg-vanilla/35 hover:bg-opacity-5"
                                  >
                                    <Link to={`/reservation/${reservation.id}/review`} className='flex flex-row items-center h-full gap-x-2'>
                                      <img
                                        src={host + "/uploads/rooms/" + reservation?.roomId + "/" + reservation?.roomCover}
                                        alt={reservation.roomName}
                                        className="w-1/3 md:max-h-[80px] rounded-md md:w-auto aspect-video"
